 ...
 * @version 1.0
 *
 * Purpose/Description: This class provides a GUI form that allows the user to
 * edit the details of an existing employee. It enables modification of
 * employee's name, surname, gender, address, department, and pay level. The
 * updated details are validated and saved into the main employee list upon
 * confirmation. Features: - Pre-loads employee information into the form
 * fields. - Allows editing of first name, last name, gender, address,
 * department, and pay level. - Validates user input for completeness and
 * correctness. - Updates the shared employee list in the main application upon
 * saving changes. - Provides cancellation to close the form without changes.
...
public class EditEmployeeForm extends javax.swing.JFrame {
    ...
    /**
     * Constructor: EditEmployeeForm Initializes the form with the existing
     * employee's information preloaded.
     *
     * @param mainWindow Reference to the main application window to access
     * shared data.
     ...
     *
     * @author Hajar Purpose: Populates the department combo box with all
     * departments available from the main window. Adds a "No Department" option
     * for employees without a department.
     *
     * Effect: Clears any existing items in the combo box and reloads them.
     */
    ...
    /**
     * Name: preSelectDepartment Purpose: Sets the selected department in the
     * combo box to match the employee's current department. If the employee has
     * no department assigned, "No Department" is selected.
     *
     * Effect: Changes the combo box selection to the employee's department.
     */
     ...
     *
     * @author Raghad Purpose: Populates the pay level combo box with predefined
     * salary levels.
     *
     * Effect: Clears existing items and adds salary levels for selection.
     */
    ...
    /**
     * Event Handler: cancelButton1ActionPerformed Purpose: Responds to Cancel
     * button click by closing the edit form without saving changes.
     *
     * Effect: Disposes the current form and returns control to the main window.
     */
    ...
    /**
     * Name: maleEditButtonActionPerformed Purpose/description: Handles the
     * action event triggered when the male gender edit button is clicked.
     * Typically used to set or update the gender field to "Male" during
     * editing.
     *
    ...
    /**
     * Name: lastNameEditFieldActionPerformed Purpose/description: Handles the
     * action event when the last name edit text field triggers an action.
    ...
    /**
     * Name: firstNameEditFieldActionPerformed Purpose/description: Handles the
     * action event when the first name edit text field triggers an action.
    ...
    /**
     * Event Handler: saveEditButtonActionPerformed Purpose: Validates user
     * input and saves the updated employee information back to the main
     * employee list.
     *
     * Steps: - Retrieves all form inputs. - Validates that all required fields
     * are filled and correctly formatted. - Converts selected department and
     * pay level to internal codes/IDs. - Updates the employee object. -
     * Replaces the employee entry in the main employee list. - Shows success or
     * error messages. - Refreshes the main window's employee table and details
     * view. - Closes the edit form.
     *
     * Exception Handling: - Shows a JOptionPane error dialog if any validation
     * fails.
     */
    private void saveEditButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveEditButtonActionPerformed
        ...
        try {
            ...
            Integer selectedDepartmentID = Department.getIdByDepartmentName(departments, selectedDepartment);
            Integer currentDeptID = selectedEmployee.getDeptID();
            Integer deptID = currentDeptID;

            boolean isDepartmentChanging
                    = (selectedDepartmentID == null && currentDeptID != null)
                    || (selectedDepartmentID != null && !selectedDepartmentID.equals(currentDeptID)); // EQUALS NOT == BECAUSE IT IS COMPARING REFERENCES INSTEAD OF VALUES 

            if (isDepartmentChanging) {
                if (selectedEmployee.isIsHead()) {
                    JOptionPane.showMessageDialog(this,
                            "Cannot change employee's department.\nEmployee is a head of a department",
                            "Input Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                if (!selectedDepartment.equals("No Department")) {
    ...
    /**
     * Name: departmentEditComboActionPerformed Purpose/description: Handles the
     * event when a department is selected from the departmentEditCombo combo
     * box. Use this method to respond to changes in the selected department,
     * such as updating related fields.
     *
    ...
    /**
     * Name: payLevelEditComboActionPerformed Purpose/description: Handles the
     * event when a pay level is selected from the payLevelEditCombo combo box.
     * Use this method to respond to changes in the selected pay level, like
     * updating salary or role information.
     *
